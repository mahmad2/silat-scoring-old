...
import { StyleSheet, View, Dimensions, Platform, StatusBar, Image, Alert } from 'react-native';
...
var paddingOntop = Platform.OS === 'ios' ? 25 : 0
...
export default class GamePage extends React.Component {
  static navigationOptions = {
    title: 'Score Pencak Silat Match',
    header: null
  };
  constructor() {
    super();
  ...
  advanceRound(){
    if(currentRound===2){
      Alert.alert("Cannot advance to another round.", "There are only THREE (3) rounds in Pencak Silat.")
    }else{
      currentRound++
      Alert.alert("ROUND "+ (currentRound+1))
      redArrayText = []
      blueArrayText = []
  ...
  addRedScoreText(score){
    ...
    if((score===-1 && minusOnePressedRed) || (score===-2 && minusTwoPressedRed) || (score===-5 && minusFivePressedRed) || (score===-10 && minusTenPressedRed)){
      switch(score) {
          case -1:
            Alert.alert("Minus one (-1) can only be pressed once per round")
            break
          case -2:
            Alert.alert("Minus one (-2) can only be pressed once per round")
            break
          case -5:
            Alert.alert("Minus one (-5) can only be pressed once for the entire match")
            break
          case -10:
            Alert.alert("Athlete in the RED corner has been disqualified. The winner is BLUE corner")
          break
      }
    } else {
    ...
    if((score===-1 && minusOnePressedBlue) || (score===-2 && minusTwoPressedBlue) || (score===-5 && minusFivePressedBlue) || (score===-10 && minusTenPressedBlue)){
      switch(score) {
          ...
          case -5:
            ...
            break
          case -10:
            Alert.alert("Athlete in the BLUE corner has been disqualified. The winner is RED corner")
          break
      }
    } else {
  ...
  comment(){
    Alert.alert("MEMO AREA", "This 'memo' area will be used for comments (remarks) and also when team makes a protest.")
  }
  ...
  openConfirm(show) {
    Alert.alert(
      ...
      [
        {text: 'Yes', onPress: () => this.optionYes()},
        ...
        return (
          <Container style={styles.container}>
            ...
            <View style={styles.middleSide}>
              ...
              <Button style={[styles.middleContents, styles.middleButtons]} full rounded warning onPress={() => this.openConfirm()}>
                <Text style={[styles.middleButtonText]}>NEW MATCH</Text>
              </Button>
              <Button style={[styles.middleContents, styles.middleButtons]} full rounded info onPress={() => navigate('SaveTabs', { redTotalScore: this.state._totalRed, blueTotalScore: this.state._totalBlue, round1Red: this.state._round1_redText, round1Blue: this.state._round1_blueText, round2Red: this.state._round2_redText, round2Blue: this.state._round2_blueText, round3Red: this.state._round3_redText, round3Blue: this.state._round3_blueText })}>
                <Text style={[styles.middleButtonText]}>SAVE SCORES</Text>
              </Button>
...
//const paddingOntop = (Platform.OS === 'ios' ? StatusBar.currentHeight : 0) //25
